...
$g->loadObject($ss);
$id=$_GET['id'];
$g->setSql("delete from l_category where id='".$id."'");
$g->query();
if($id)
 { 
echo "<script>location.href='category.php';</script>"; 
}
$num_query=mysql_query("select count(id) from l_category ");//查询数据库总条数
$pagesize=10;//定义每个页面显示的条数
$category=mysql_fetch_array($num_query);
$count=$category[0];//总条数
$page=empty($_GET['page'])?1:$_GET["page"];//显示当前页
//显示当页面信息页面
$sql="select * from l_category limit ".($page-1)*$pagesize.",".$pagesize;
...
<ul class="left_menu_a">
    ...
    <ul>
		<li>
            ...
            <i class="fa fa-list-ul fa-fw"></i> 分类管理<i class="fa fa-chevron-up fa-fw"></i></div>
			<ul class="left_menu_sub1">
				<li class="sub1_color"><a href="category.php">所有分类</a></li>
			</ul>
		</li>
	</ul>
   ...
   <!-- <div class="article_function">
	...
	<table id="article_table" border="0" cellpadding="0" cellspacing="0">
		...
		echo "<tr>";
			...
			echo '<td>
					<a href="category.php?id='.$category[0].'">删除</a>
				 </td>';
			echo "</tr>";
		}	
			}else{
	echo "<td>暂时没有内容</td>";
		...
		?>
 <!--新增分类-->
<form id="news" method = "post" onsubmit = "return checknew();" name="newc"/ > 

		 <tr>
			<td></td>
    		<td></td>
    		<td></td>
            <td><input type="text" class="article_input" id="category" name="category" placeholder="新分类" required maxLength="12"></td>
    		<td></td>
            <td><input class="article_button" type="submit" value="增加" name="newc"/></td>
		</tr>  
