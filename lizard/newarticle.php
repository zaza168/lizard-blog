...
<article>
    ...
    <form id="news" method = "post" onsubmit = "return checknew();" name="newa"/ > 
	题目：<input type="text" class="article_input" id="title" name="title" placeholder="未命名" >
    分类：
    ...
    状态：
		...
		<option value="1" name="state">正常</option>
        <option value="0" name="state">关闭</option>
		</select>	


    <input class="article_button" type="submit" value="提交" name="newa"/>
    </div>
    <div class="article_main">
    正文：（字数统计：<span class="word_count">0</span>）
    <textarea id="editor_id" name="article"><?php echo htmlspecialchars($htmlData); ?> </textarea>
