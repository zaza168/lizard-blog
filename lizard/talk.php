...
$g->loadObject($ss);
$id=$_GET['id'];
$g->setSql("delete from l_talk where id='".$id."'");
$g->query();
if($id)
 { 
echo "<script>location.href='talk.php';</script>"; 
}
$num_query=mysql_query("select count(id) from l_talk");//查询数据库总条数
$pagesize=5;//定义每个页面显示的条数
$talk=mysql_fetch_array($num_query);
$count=$talk[0];//总条数
$page=empty($_GET['page'])?1:$_GET["page"];//显示当前页
//显示当页面信息页面
$sql="select * from l_talk limit ".($page-1)*$pagesize.",".$pagesize;
    ...
    <ul>
		<li>
			...
			<ul class="left_menu_sub1">
				<li class="sub1_color"><a href="talk.php">所有微博</a></li>
   ...
   <!-- <div class="article_function">
	...
	<table id="article_table" border="0" cellpadding="0" cellspacing="0">
		...
		echo "<tr>";
			...
			echo '<td>'.$talk[2].'</td>';
			echo '<td>
					<a href="edittalk.php?id='.$talk[0].'">编辑</a>/
					<a href="talk.php?id='.$talk[0].'">删除</a>
				 </td>';
			echo "</tr>";
		}	
			}else{
